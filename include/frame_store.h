...
private:
    ...
    cv::Mat last_; // - last_: последний полученный кадр.
    bool has_frame_ = false; // - has_frame_: признак наличия первого кадра.

    // seq_ меняется только под mutex. Это "версия" кадра.
    // Потребитель ждёт, пока seq_ станет больше локально сохранённого.
    uint64_t seq_ = 0; // - seq_: счетчик версий для сигнализации новых кадров.

    mutable std::mutex m_; // - m_: мьютекс для защиты состояния.
    std::condition_variable cv_; // - cv_: условная переменная для ожидания кадра.
    bool stop_ = false; // - stop_: флаг остановки ожиданий.
};
